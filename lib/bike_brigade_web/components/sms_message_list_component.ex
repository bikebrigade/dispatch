defmodule BikeBrigadeWeb.Components.SMSMessageListComponent do
  ...
  def handle_build_message_link({"/messages", rider_id}) do
    ~p"/messages/#{rider_id}"
  end

  def handle_build_message_link({"/campaign", campaign_id, rider_id}) do
    ~p"/campaigns/#{campaign_id}/messaging/riders/#{rider_id}"
  end


  # TODO: can :any be a tuple?
  # TODO: fix indent.
  ...
  def message_link(assigns) do
    route = case assigns.sms_list do
      {"/messages"} -> ~p"/messages/#{assigns.rider_id}"
      {"/campaigns", campaign_id} -> ~p"/campaigns/#{campaign_id}/messaging/riders/#{assigns.rider_id}"
    end
