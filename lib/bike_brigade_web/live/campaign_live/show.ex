defmodule BikeBrigadeWeb.CampaignLive.Show do
  ...
  defp apply_action(socket, :add_rider, _) do
    ...
    |> assign(:rider, %CampaignRider{})
  end

  defp apply_action(socket, :edit_rider, %{"rider_id" => rider_id, "id" => campaign_id}) do
    fetched_rider = Delivery.get_campaign_rider_by_id(campaign_id, rider_id)
  ...
  defp apply_action(socket, :campaign_messaging_index, _params) do
    socket
    |> assign(:page_title, "Campaign Messages")
    |> assign(:selected_rider_chat_id, nil)
  end

  defp apply_action(socket, :campaign_messaging_show, %{"rider_id" => rider_id_str}) do
    rider_id = String.to_integer(rider_id_str)
