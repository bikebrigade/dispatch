defmodule BikeBrigadeWeb.Components.UserSelectionComponent do
  ...
  @impl Phoenix.LiveComponent
  def update(%{selected_user_id: selected_user_id} = assigns, socket) when not is_nil(selected_user_id) do
  ...
  def render(assigns) do
    ...
    <div>
    <%= if @selected_user do %>
    <a href="#" phx-click="unselect" phx-target={@myself} class="block transition duration-150 ease-in-out hover:bg-gray-50 focus:outline-none focus:bg-gray-50">
      <div class="flex items-center px-4 py-4 sm:px-6">
        <div class="flex items-center flex-1 min-w-0">
          ...
          <div class="flex-1 min-w-0 px-4 md:grid md:grid-cols-2 md:gap-4">
            <div>
              <div class="text-sm font-medium leading-5 text-indigo-600 truncate"><%= @selected_user.name %>
              </div>
    ...
    <ul id="user-selection-list" class="overflow-y-auto max-h-64">
      <%= for user <- @users do %>
        ...
        <a href="#" phx-click="select" phx-value-id={user.id} phx-target={@myself} class="block transition duration-150 ease-in-out hover:bg-gray-50 focus:outline-none focus:bg-gray-50">
          <div class="flex items-center px-4 py-4 sm:px-6">
            <div class="flex items-center flex-1 min-w-0">
              <div class="flex-shrink-0">
              ...
              <div class="flex-1 min-w-0 px-4 md:grid md:grid-cols-2 md:gap-4">
                <div>
                  <div class="text-sm font-medium leading-5 text-indigo-600 truncate"><%= user.name %>
                  </div>
                  <div class="flex items-center mt-2 text-sm leading-5 text-gray-500">
                    <svg class="flex-shrink-0 mr-1.5 h-5 w-5 text-gray-400" viewBox="0 0 20 20" fill="currentColor">
                      <path d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"></path>
